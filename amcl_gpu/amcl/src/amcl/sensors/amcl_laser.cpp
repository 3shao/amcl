...
AMCLLaser::SetModelLikelihoodField(double z_hit,
                                   ...
                                   double max_occ_dist)
{
  ROS_INFO("[LASER AMCL]: AMCLLaser::SetModelLikelihoodField");
...
{
  ROS_INFO("[LASER AMCL]: AMCLLaser::SetModelLikelihoodFieldProb");
...
{
  ROS_INFO("[LASER AMCL]: AMCLLaser::UpdateSensor");
  if (this->max_beams < 2)
    return false;
...
{
  ROS_INFO("[LASER AMCL]: AMCLLaser::BeamModel");
  AMCLLaser *self;
  int i, j, step;
  double z, pz;
  double p;
...
{
  ROS_INFO("[LASER AMCL]: AMCLLaser::LikelihoodFieldModel");
  AMCLLaser *self;
  int i, j, step;
  double z, pz;
  double p;
  ...
  for (j = 0; j < set->sample_count; j++)
  {
    sample = set->samples + j;
    pose = sample->pose;
//    ROS_INFO("check0 i:%d laser_pose.v[0]:%lf laser_pose.v[1]:%lf laser_pose.v[2]:%lf v[0]:%lf v[1]:%lf v[2]:%lf \n",
//				  j, self->laser_pose.v[0], self->laser_pose.v[1], self->laser_pose.v[2], pose.v[0], pose.v[1], pose.v[2]);

    // Take account of the laser pose relative to the robot
    pose = pf_vector_coord_add(self->laser_pose, pose);
//    ROS_INFO("check1 i:%d v[0]:%lf v[1]:%lf v[2]:%lf \n",
//				  j, pose.v[0], pose.v[1], pose.v[2]);
...
//					j, i, hit.v[0], hit.v[1], pose.v[0], pose.v[1], pose.v[2], obs_range, obs_bearing);

      // Convert to map grid coords.
      int mi, mj;
      mi = MAP_GXWX(self->map, hit.v[0]);
      mj = MAP_GYWY(self->map, hit.v[1]);
//	  ROS_INFO("check4 j:%d  hit.v[0]:%lf hit.v[1]:%lf mi:%d mj:%d", i, hit.v[0], hit.v[1], mi, mj);
...
//  	  ROS_INFO("check5 j:%d x:%d, y:%d mi:%d mj:%d index:%d z:%lf", i, self->map->size_x, self->map->size_y, mi, mj, MAP_INDEX(self->map,mi,mj), z);
    ...
    sample->weight *= p;
    total_weight += sample->weight;
//    ROS_INFO("check8 :total_weight:%lf samples[].weight:%lf p:%lf", total_weight, sample->weight, p);
  }
#else
  sensor_gpu_LikelihoodFieldModel(set, self, data, &total_weight);
  ROS_INFO("[LASER AMCL DEBUG] :total_weight:%f", total_weight);
#endif
...
{
  ROS_INFO("[LASER AMCL]: AMCLLaser::LikelihoodFieldModelProb");
  AMCLLaser *self;
  int i, j, step;
  double z, pz;
  double log_p;
...
void AMCLLaser::reallocTempData(int new_max_samples, int new_max_obs){
  ROS_INFO("[LASER AMCL]: AMCLLaser::reallocTempData");
  if(temp_obs){
    for(int k=0; k < max_samples; k++){
      delete [] temp_obs[k];
    }
