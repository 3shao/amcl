...
{
  ROS_INFO("[AMCL]: AmclNode::AmclNode IN");
  ...
  odometry_sub_ = nh_.subscribe("odom", 2, &AmclNode::odometryReceived, this);
  ...
  ROS_INFO("[AMCL]: AmclNode::AmclNode OUT");
}

void AmclNode::reconfigureCB(AMCLConfig &config, uint32_t level)
{
  ROS_INFO("[AMCL]: AmclNode::reconfigureCB IN");
  ...
  initial_pose_sub_ = nh_.subscribe("initialpose", 2, &AmclNode::initialPoseReceived, this);
  ROS_INFO("[AMCL]: AmclNode::reconfigureCB OUT");
}


void AmclNode::runFromBag(const std::string &in_bag_fn)
{
  ROS_INFO("[AMCL]: AmclNode::runFromBag IN");
  rosbag::Bag bag;
  ...
  ros::shutdown();
  ROS_INFO("[AMCL]: AmclNode::runFromBag OUT");
}


void AmclNode::savePoseToServer()
{
  ROS_INFO("[AMCL]: AmclNode::savePoseToServer IN");
...
{
  ROS_INFO("[AMCL]: AmclNode::updatePoseFromServer IN");
  init_pose_[0] = 0.0;
  init_pose_[1] = 0.0;
  init_pose_[2] = 0.0;
  init_cov_[0] = 0.5 * 0.5;
...
{
  ROS_INFO("[AMCL]: AmclNode::checkLaserReceived IN");
  ...
  ROS_INFO("[AMCL]: AmclNode::checkLaserReceived OUT");
}
...
AmclNode::requestMap()
{
  ...
  ROS_INFO("[AMCL]: AmclNode::requestMap IN");
  // get map via RPC
  ...
  ROS_INFO("[AMCL]: AmclNode::requestMap OUT");
}

void
AmclNode::mapReceived(const nav_msgs::OccupancyGridConstPtr& msg)
{
  ROS_INFO("[AMCL]: AmclNode::mapReceived IN");
  ...
  ROS_INFO("[AMCL]: AmclNode::mapReceived OUT");
}
...
AmclNode::handleMapMessage(const nav_msgs::OccupancyGrid& msg)
{
  ...
  ROS_INFO("[AMCL]: AmclNode::handleMapMessage IN");
  ...
  applyInitialPose();
  ROS_INFO("[AMCL]: AmclNode::handleMapMessage OUT");
...
{
  ROS_INFO("[AMCL]: AmclNode::convertMap IN");
  map_t* map = map_alloc();
  ROS_ASSERT(map);
  ...
  ROS_INFO("[AMCL]: AmclNode::convertMap OUT");
...
AmclNode::getOdomPose(tf::Stamped<tf::Pose>& odom_pose,
                      ...
                      const ros::Time& t, const std::string& f)
{
  ROS_INFO("[AMCL]: AmclNode::getOdomPose IN");
  ...
  ROS_INFO("[AMCL]: AmclNode::getOdomPose OUT");
...
{
  map_t* map = (map_t*)arg;
  ROS_INFO("[AMCL]: AmclNode::uniformPoseGenerator IN");
#if NEW_UNIFORM_SAMPLING
...
#endif
  ROS_INFO("[AMCL]: AmclNode::uniformPoseGenerator OUT");
  return p;
}
...
AmclNode::globalLocalizationCallback(std_srvs::Empty::Request& req,
                                     std_srvs::Empty::Response& res)
{
  ROS_INFO("[AMCL]: AmclNode::globalLocalizationCallback IN");
  if( map_ == NULL ) {
    return true;
  }
  ...
  pf_init_ = false;
  ROS_INFO("[AMCL]: AmclNode::globalLocalizationCallback OUT");
  return true;
}
...
AmclNode::setMapCallback(nav_msgs::SetMap::Request& req,
                         nav_msgs::SetMap::Response& res)
{
  ROS_INFO("[AMCL]: AmclNode::setMapCallback IN");
  handleMapMessage(req.map);
  handleInitialPoseMessage(req.initial_pose);
  res.success = true;
  ROS_INFO("[AMCL]: AmclNode::setMapCallback OUT");
  return true;
}
...
{
  ROS_INFO("[AMCL]: AmclNode::laserReceived IN");
  ...
  if(lasers_update_[laser_index])
  {
    ...
    ROS_INFO("[AMCL]: Num samples: %d", set->sample_count);
  ...
  ROS_INFO("[AMCL]: AmclNode::laserReceived OUT");
}
...
{
  ROS_INFO("[AMCL]: AmclNode::initialPoseReceived IN");
  handleInitialPoseMessage(*msg);
  ROS_INFO("[AMCL]: AmclNode::initialPoseReceived OUT");
}

void
AmclNode::handleInitialPoseMessage(const geometry_msgs::PoseWithCovarianceStamped& msg)
{
  ROS_INFO("[AMCL]: AmclNode::handleInitialPoseMessage IN");
  ...
  initial_pose_hyp_->pf_pose_cov = pf_init_pose_cov;
  applyInitialPose();
  ROS_INFO("[AMCL]: AmclNode::handleInitialPoseMessage OUT");
}
...
AmclNode::applyInitialPose()
{
  ...
  ROS_INFO("[AMCL]: AmclNode::applyInitialPose OUT");
}
